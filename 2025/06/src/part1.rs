...
pub fn solve(input: &ParsedInput) -> u64 {
    let mut result = 0;
...
pub fn parse_input<S: AsRef<str>>(lines: impl Iterator<Item = S>) -> Result<ParsedInput, String> {
    ...
    for line in lines {
        ...
        if Operation::from_str(first_word).is_ok() {
            ...
            let parsed_numbers: Vec<u64> = words
                .map(|word| {
                    word.parse().map_err(|err| {
                        format!("could not parse word \"{word}\" as number: {err:?}")
                    })
                })
                .collect::<Result<_, _>>()?;
...
*   +   *   +  ";
        ...
        assert_eq!(solve(&parsed_input), 4277556);
    }
}
